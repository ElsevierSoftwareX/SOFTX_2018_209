...
class RunUlangLagiCommand extends Service {
    public function actionIndex() {
        ## Put your code here
        $datas = Yii::app()->db->createCommand('SELECT * FROM file WHERE is_ok ISNULL ORDER BY id')->queryAll();
        foreach($datas as $data){
            ...
            if($do){
                ...
                if(Yii::app()->db->createCommand('SELECT COUNT(1) FROM c_mapping_file WHERE file_id = '.$id.' LIMIT 1')->queryScalar()>0){
                    Yii::app()->db->createCommand('UPDATE file SET is_ok = 1 WHERE id = '.$id)->execute();
                    echo 'Success<br/>';
                }
                else echo 'Failed<br/>';
